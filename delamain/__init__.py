...
def _nextdns_get_records() -> list[dict]:
    r = requests.get(
        f"https://api.nextdns.io/profiles/{settings.nextdns_profile_id}/rewrites/",
...
def _nextdns_delete_record(next_dns_id: str) -> None:
    r = requests.delete(
        f"https://api.nextdns.io/profiles/{settings.nextdns_profile_id}/rewrites/{next_dns_id}",
        headers={"X-Api-Key": settings.nextdns_api_key},
        timeout=5,
    ).raise_for_status()
    logger.debug(r.text)
...
def run() -> None:
    ...
    for lookup in settings.lookups.split(","):
